package tech.mystox.framework.mq.core;
...
public class ExecutorConfig extends CommonExecutorConfig {
    ...
    {
        return builder(CORE_POOL_SIZE, MAX_POOL_SIZE, 5000, 30000, "mqExecutor-");
    }

    @Bean(name = "mqScheduled")
    ScheduledExecutorService mqttScheduled() {
        return Executors.newScheduledThreadPool(10);
    }

    @Bean(name = "mqSenderAckExecutor")
    public ThreadPoolTaskExecutor mqttAck()
    {
        return builder(CORE_POOL_SIZE, MAX_POOL_SIZE, 2000, 10000, "mqAck-");
    }
