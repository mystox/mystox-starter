...
//    private int maxInflight;
     ...
     * 订阅的bean名称
     */
    public static final String CHANNEL_NAME_IN = "kafkaInboundChannel";
    /**
     * 发布的bean名称
     */
     ...
     * 回复通道
     */
    public static final String CHANNEL_REPLY = "kafkaReplyBoundChannel";
    ...
    public MessageHandler kafkaOutbound(KafkaTemplate kafkaTemplate) {
    ...
    public ConsumerFactory kafkaConsumerFactory(KafkaProperties properties) {
        ...
        consumerProperties.put(ConsumerConfig.METADATA_MAX_AGE_CONFIG, 500);
        return new DefaultKafkaConsumerFactory<>(consumerProperties);
    }

    @Bean
    public KafkaMessageListenerContainer container(@Qualifier(value = "myKafkaConsumerFactory") ConsumerFactory kafkaConsumerFactory) {
    ...
    adapter(KafkaMessageListenerContainer<String, String> container) {
        ...
        kafkaMessageDrivenChannelAdapter.setOutputChannel(kafkaInboundChannel());
        return kafkaMessageDrivenChannelAdapter;
    }
    @Bean("replyProducer")
    public MessageProducer replyProducer(@Qualifier(value = "myKafkaConsumerFactory") ConsumerFactory kafkaConsumerFactory) {
        KafkaMessageDrivenChannelAdapter<String, String> kafkaMessageDrivenChannelAdapter =
                new KafkaMessageDrivenChannelAdapter<>(new KafkaMessageListenerContainer<>( kafkaConsumerFactory,
                        new ContainerProperties(new TopicPartitionInitialOffset("reply_topic", 0))));
        kafkaMessageDrivenChannelAdapter.setOutputChannel(kafkaInboundChannel());
        return kafkaMessageDrivenChannelAdapter;
    }
    @Bean
