package com.kongtrolink.framework.entity;

public class WebMessage {

    public final static String NOT_FOUND_SN = "未找到sn对应终端信息";
    public final static String NOT_FOUND_FSU_ID = "未找到对应FsuId信息";
    public final static String NOT_FOUND_DEVICE = "未找到对应设备";
    public final static String NOT_FOUND_DEVICE_ID = "未找到设备Id";
    public final static String NOT_FOUND_DEVICE_TYPE = "未找到设备类型信息";
    public final static String NOT_FOUND_SIGNAL = "未找到设备对应信号点";
    public final static String NOT_FOUND_SIGNAL_LIST = "未找到设备对应信号点列表";
    public final static String NOT_FOUND_ALARM = "未找到设备对应告警点";
}
