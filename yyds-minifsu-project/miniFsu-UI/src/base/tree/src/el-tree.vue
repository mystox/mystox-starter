...
export default {
  ...
  methods: {
    ...
    closeAllNodes () {
      this.store.toggleAllNodes(false)
    },
    expandllNodes () {
      this.store.toggleAllNodes(true)
    },
    getNode (arg) {
      return this.store.getNode(arg)
    },
    setAllNodeChecked (value) {
      window.setTimeout(() => {
        this.store.setAllNodeChecked(value)
      }, 0)
    },
    checkFirst () {
      const traverse = (n) => {
        ...
        if (n.isLeaf) {
          this.setChecked(n.key, true)
        } else {
          traverse(n.childNodes[0])
        }
      }
      traverse(this.root)
    }
  ...
  created () {
    ...
    this.$once('tree-ready', this.onReady)
  },
  mounted () {
    this.initTabindex()
