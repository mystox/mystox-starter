...
(function () { 
    ...
    *@return 无返回值。
    */
    TextIconOverlay.prototype.setText = function(text, level, size){
    ...
    */
    TextIconOverlay.prototype.getStyleByText = function(text, styles, level){
        var count = parseInt(text);
        var index = parseInt(count / 10);
        index = Math.max(0, index);
        index = Math.min(index, styles.length / 4 - 1);
        return styles[index + level * 5] || {}; 
    };
    ...
    */
    TextIconOverlay.prototype._updateCss = function(){
        ...
        var style = this.getStyleByText(this._text, this._styles, level);
        this._domElement.style.cssText = this._buildCssText(style, this._text > 0);
        this._domElement.className = 'alarm-icon level' + level;
    };
    ...
    *@return 无返回值。
    */
    TextIconOverlay.prototype._updateText = function(text){
        if (this._domElement) {
            this._domElement.innerHTML = '<div class="alarm-number-group">' + text + '</div>';
        }
    };
    ...
    */
    TextIconOverlay.prototype._buildCssText = function(style, inject) {    
        //根据style来确定一些默认值
        var url = style['url'];
        var size = style['size'];
        ...
        /* Amas Added*/
        if (inject) {
            csstext.push('pointer-events:none;');
        }
        /* /Amas Added*/
        return csstext.join('');
    };
