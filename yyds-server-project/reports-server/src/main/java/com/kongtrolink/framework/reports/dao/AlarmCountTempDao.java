...
public class AlarmCountTempDao extends MongoBaseDao {
    ...
    public List<JSONObject> getCountDataByCondition(String taskId, JSONObject condition, TimePeriod timePeriod) {
        ...
        if (!"全部".equals(stationType)&& StringUtils.isNotBlank(stationType)) criteria.and("stationType").is(stationType);
        String runningSate = condition.getString("operationState");
        if (!"全部".equals(runningSate) &&  StringUtils.isNotBlank(runningSate)) criteria.and("operationState").is(runningSate);
        String fsuManufactory = condition.getString("fsuManufactory");
        if (!"全部".equals(fsuManufactory) &&  StringUtils.isNotBlank(fsuManufactory)) criteria.and("fsuManufactory").is(fsuManufactory);
