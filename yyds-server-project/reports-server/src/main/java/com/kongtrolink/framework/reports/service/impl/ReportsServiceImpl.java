    ...
    @ReportOperaCode(code = OperaCodePrefix.REPORTS + "config", rhythm = 20, resultType = {DataType.JSON}, extend =
            {
                    ...
                    @ReportExtend(field = "startTime", name = "开始时间", type = ReportExtend.FieldType.DATE), //时间类型是否需要(查询用)
                    @ReportExtend(field = "endTime", name = "结束时间", type = ReportExtend.FieldType.DATE), //时间类型是否需要（查询用）
