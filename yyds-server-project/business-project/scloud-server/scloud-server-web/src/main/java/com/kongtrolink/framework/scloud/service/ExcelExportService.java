...
public class ExcelExportService {
    ...
    public static String exportTwoLineHeadData(HttpServletResponse response, String fileName,
        ...
        try {
            ...
            initData(wb, sheet, data, properiesName, 2);
     ...
     * 功能描述:填充数据
     */
    private static void initData(SXSSFWorkbook workbook, Sheet sheet, List data, String[] properiesName, int beginLine){
        //填充数据
        Row row ;
        Cell cell;
        //设置单元格样式
        ...
        for(int i=0; i<data.size(); i++){
            //创建行
            row = sheet.createRow(i + beginLine);
    ...
    public static String alarmSiteHistoryExport(HttpServletResponse response, String fileName,  List<AlarmSiteStatistics> businessList){
        ...
        try {
            ...
            sheet.setDefaultColumnWidth(20);
            //拼接表头
            alarmSiteHistoryHead(workbook, sheet, businessList.get(0).getProperties(), "告警");
            //设置单元格样式
            CellStyle cellStyle = workbook.createCellStyle();
            cellStyle.setAlignment(HSSFCellStyle.ALIGN_CENTER); //水平居中
            cellStyle.setVerticalAlignment(HSSFCellStyle.VERTICAL_CENTER);//垂直居中
    ...
    private static void alarmSiteHistoryHead(SXSSFWorkbook workbook, Sheet sheet, List<String> properties, String tableTitle){
        //设置单元格基本样式
        CellStyle cellStyle = getHeaderStyle(workbook);
        //创建两行头
        //先创建两行表头需要的单元格2行，9列，行和列下标都从0开始
        ...
        //站点
        Cell cell0_0 = row_0.getCell(0);
        cell0_0.setCellValue(properties.get(0));
        cell0_0.setCellStyle(cellStyle);
        sheet.addMergedRegion(new CellRangeAddress(0, 1, 1, 1));
        //告警总数
        Cell cell0_1 = row_0.getCell(1);
        cell0_1.setCellValue(properties.get(1));
        cell0_1.setCellStyle(cellStyle);
        sheet.addMergedRegion(new CellRangeAddress(0, 0, 2, properties.size()-2));
        Cell cell0_2 = row_0.getCell(2);
        cell0_2.setCellValue(tableTitle);
    ...
    private static void initData(Sheet sheet, CellStyle cellStyle, List<String> values, int lineNum){
        //创建行
