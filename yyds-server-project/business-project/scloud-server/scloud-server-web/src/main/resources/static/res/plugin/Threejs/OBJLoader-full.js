...
THREE.OBJLoader.prototype = {
	...
	load: function ( url, onLoad, onProgress, onError ) {
		...
		THREEx.Cache.queryCache('obj', url, function (res) {
			var loader = null;
			if (res) {
				// setTimeout(function () {
				// 	onLoad({children:{length:0}});// scope.parse( res.data ) );
				// },0)

				// loader = new THREE.XHRLoader( scope.manager );
				// loader.setPath("");
				// loader.load( URL.createObjectURL(new Blob([res.data])), function ( text ) {
				...
				loader.load( url, function ( text ) {
					...
					THREEx.Cache.addCache('obj', {url: url, data: text});
					onLoad( scope.parse( text ) );
				...
				// 	THREEx.Cache.addCache('image', {url: url, data: blob});
