...
## 简介
*mystox‑starter* 是一款**专为移动端和边缘设备设计的轻量级 MQ‑RPC 框架**，充分优化了点对点通信与灵活 RPC 调用，适合网络环境复杂或设备间协作频繁的场景。
...
- **点对点互通**  
  不再依赖公网 IP 与传统服务注册中心，轻松实现设备间的互联通信，适用 IoT、无人机、工业设备等移动场景。

- **MQ 驱动 RPC**  
  基于 Kafka/MQTT 等消息队列实现，天然支持异步请求、响应以及推送消息，具备优秀的解耦性和可扩展性。

- **断线重连 & 高可用**  
  内置重试机制与连接管理，使得通信更加稳定可靠，即使遇断线或弱网也可快速恢复。

- **轻量高效**  
  框架为移动设备量身定制，部署轻量，运行高效，适合资源受限的终端环境。
...
| 场景类型              | 使用价值说明 |
|-----------------------|--------------|
| 边缘设备协同         | IoT 或传感集群中设备间需RPC交互 |
| 弱网环境通信         | 移动、无人机等场景下，网络不稳定仍可可靠通信 |
| 动态设备拓展         | 临时加入的终端设备能自动接入系统 |
| 安全私有网络通信     | 无需暴露公网，降低安全风险 |

---


## 为什么选择 mystox‑starter？

- **专攻移动/边缘场景**：不同于通用 RPC 框架，*mystox‑starter* 从架构到实现都针对移动终端优化。
- **简化网络复杂性**：封装 MQ 通信细节，开发者专注接口定义，不再关心底层网络障碍。
- **自主可控，易于扩展**：无需第三方注册中心，具备云/本地部署灵活性，可按需定制认证、安全、监控等组件。
...
* yyds-iarpc-starter: springboot适配模块
